...
void get(int argc, char * argv[], const char ** filename, const char ** out_filename, const char ** rebuild_filename, long *offset, long *block_size, long *data_size, int *channels, int *version, bool *strict, bool *verbose);
}
...
int main(int argc, char * argv[]) {
    ...
    cout << "XMA/XMA2 stream parser 0.8 by hcs" << endl << endl << flush;

    try {
        Get_arguments::get(argc, argv, &input_filename, &output_filename, &rebuild_filename, &offset, &block_size, &data_size, &channels, &version, &strict, &verbose);
...
<< endl;
    ...
    try {
        ...
        if (version == 1) {
            ...
            {
                total_sample_count = Parse_XMA::parse_XMA_packets(is, os, offset, data_size, (channels > 1), strict, verbose);
            }
            
            if (rs.is_open()) {
                total_sample_count = Parse_XMA::build_XMA_from_XMA(is, out_bitstream, offset, data_size, &ctx, (channels > 1), strict, verbose);
            }
        } else {
            for (long block_offset = offset;
                ...
                if (!rs.is_open() || os.is_open()) {
                    sample_count = Parse_XMA::parse_XMA2_block(is, os, block_offset, usable_block_size, (channels > 1), strict, verbose);
                }

                if (rs.is_open()) {
                    sample_count = Parse_XMA::build_XMA_from_XMA2_block(is, out_bitstream, block_offset, usable_block_size, &ctx,
                            (channels > 1), strict, (block_offset + block_size >= offset + data_size), verbose);
                }
...
void Get_arguments::get(int argc, char * argv[], const char ** filename, const char ** out_filename, const char ** rebuild_filename, long *offset, long *block_size, long *data_size, int *channels, int *version, bool *strict, bool *verbose) {
    if (argc < 2) {
